...
from code.btwm_head import (
    InverseActionHead, inverse_accuracy, inverse_confidence, inverse_loss,
    masked_mean, transition_targets)
...
class RSSM(nj.Module):
  ...
  free_nats: float = 1.0
  use_btwm: bool = False
  inv_loss_weight: float = 0.05
  inv_loss_schedule: str = 'const'
  ...
  inv_depth: int = 2
  inv_feature_source: str = 'tokens'
  inv_normalize_loss: bool = True
  inv_confidence_gating: bool = False
  ...
  def loss(self, carry, tokens, acts, reset, training, update_step=0):
    metrics = {}
    ...
    inv_loss_weight = self._inv_loss_weight(update_step)
    if self.use_btwm:
      if self.inv_feature_source == 'tokens':
        inv_features = tokens
      elif self.inv_feature_source == 'stoch':
        inv_features = feat['stoch'].reshape(
            (*feat['stoch'].shape[:-2], -1))
      else:
        raise ValueError(
            f'Unsupported inverse feature source: {self.inv_feature_source}')
      # acts contains the previous action for each latent state. Therefore,
      # transition z_t -> z_{t+1} is caused by acts[t + 1].
      target_acts, valid = transition_targets(acts, reset)
      detached_logits = self.inv_head(
          sg(inv_features[:, :-1]), sg(inv_features[:, 1:]))
      inv_logits = self.inv_head(
          inv_features[:, :-1], inv_features[:, 1:])
      inv_loss_val = inverse_loss(inv_logits, target_acts, self.act_space)
      inv_objective = inverse_loss(
          inv_logits, target_acts, self.act_space,
          normalize=self.inv_normalize_loss)
      head_objective = inverse_loss(
          detached_logits, target_acts, self.act_space,
          normalize=self.inv_normalize_loss)
      confidence = sg(inverse_confidence(
          detached_logits, target_acts, self.act_space))
      rep_weight = sg(jnp.where(
          self.inv_confidence_gating, confidence, jnp.ones_like(confidence)))
      inv_acc = inverse_accuracy(inv_logits, target_acts, self.act_space)
      metrics['inv/ce'] = masked_mean(inv_loss_val, valid)
      metrics['inv/objective'] = masked_mean(inv_objective, valid)
      metrics['inv/head_objective'] = masked_mean(head_objective, valid)
      metrics['inv/confidence'] = masked_mean(confidence, valid)
      metrics['acc/inv'] = masked_mean(inv_acc, valid)
      ...
      metrics['inv/confidence_gating'] = jnp.asarray(
          self.inv_confidence_gating, f32)
      metrics['inv/effective_rep_weight'] = (
          inv_loss_weight * masked_mean(rep_weight, valid))

      # Agent.loss averages the full (B, T) tensor. Rescale valid transition
      # entries so that this remains the mean over valid transitions only.
      count = valid.sum()
      scale = dyn.size / jnp.maximum(count, 1)
      loss_val = (
          self.inv_head_loss_weight * head_objective +
          inv_loss_weight * rep_weight * inv_objective)
      loss_val = jnp.where(valid, loss_val, 0) * scale
      losses['inv'] = jnp.pad(loss_val, ((0, 0), (1, 0)))
      state_confidence = jnp.where(valid, confidence, 1.0)
      metrics['_inv_confidence'] = jnp.pad(
          state_confidence, ((0, 0), (0, 1)), constant_values=1.0)
    else:
      losses['inv'] = jnp.zeros_like(dyn)
      metrics['_inv_confidence'] = jnp.ones_like(dyn)
    return carry, entries, losses, feat, metrics

  def _inv_loss_weight(self, update_step):
    base = jnp.asarray(self.inv_loss_weight, f32)
    if self.inv_loss_schedule == 'const':
      return base
    if self.inv_loss_schedule != 'linear':
      raise ValueError(
          f'Unsupported inverse loss schedule: {self.inv_loss_schedule}')
    if self.inv_loss_decay_steps <= 0:
      return base
