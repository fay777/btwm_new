...
from . import rssm
from code.btwm_head import relative_confidence_weights
...
class Agent(embodied.jax.Agent):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    dyn_config = dict(config.dyn[config.dyn.typ])
    for key in (
      ...
      try:
        dyn_config[key] = config[key]
      except (KeyError, TypeError, AttributeError):
        pass
    self.dyn = {
        'rssm': rssm.RSSM,
    }[config.dyn.typ](act_space, **dyn_config, name='dyn')
    self.dec = {
        'simple': rssm.Decoder,
  ...
  def loss(self, carry, obs, prevact, training):
    ...
    dyn_carry, dyn_entries, los, repfeat, mets = self.dyn.loss(
        ...
        update_step=self.opt.step.read())
    losses.update(los)
    inv_confidence = mets.pop('_inv_confidence')
    metrics.update(mets)
    ...
    if self.config.inv_policy_weight and self.config.inv_loss_weight:
      confidence = inv_confidence[:, -K:].reshape((B * K,))
      confidence_weight = relative_confidence_weights(
          confidence,
          self.config.inv_policy_min,
          self.config.inv_policy_max)
      confidence_weight = (
          1 + self.config.inv_policy_weight * (confidence_weight - 1))
    else:
      confidence_weight = jnp.ones((B * K,), f32)
    los, imgloss_out, mets = imag_loss(
        ...
        update=training,
        policy_weight=confidence_weight,
...
def imag_loss(
  ...
  if policy_weight is not None:
    policy_weight = sg(jnp.asarray(policy_weight, f32))
    assert policy_weight.shape == (policy_loss.shape[0],), (
        policy_weight.shape, policy_loss.shape)
    policy_loss *= policy_weight[:, None]
    metrics['inv/policy_weight_mean'] = policy_weight.mean()
    metrics['inv/policy_weight_std'] = policy_weight.std()
    metrics['inv/policy_weight_min'] = policy_weight.min()
    metrics['inv/policy_weight_max'] = policy_weight.max()
